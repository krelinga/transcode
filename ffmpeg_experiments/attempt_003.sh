...
readonly log_file_output="$(dirname "$0")/attempt003.log"
...
# Info on aspect ratio: https://www.youtube.com/watch?v=hLCNqx_MJtM
export FFREPORT="file=${log_file_output}:level=32"  # 32 is an alias for INFO
"${ffmpeg}" -y -i "${input_file}" -vcodec libx264 -acodec copy -scodec copy -map 0 "${output_file}" -aspect 16:9
